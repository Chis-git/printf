...
int write_num(int ind, char buffer[], int flags, int width, int prec, int length, char padd, char extra_c)
{
	...
	if (width > length)
	{
		...
		return (write(1, &buffer[padd_start], i - padd_start) + write(1, &buffer[ind], length - ( - padd_start)));
	}
	if (extra_c)
		buffer[--ind] = extra_c;
...
int write_pointer(char buffer[], int ind, int length, int width, int flags, char extra_c, int padd_start)
{
	int i;
	if ( width > length)
	{
		...
		else if (!(flags & F_MINUS) && padd == ' ')/* extra char to left o padd */
		{
			if (extra_c)
				buffer[--padd_start] = extra_c;
			buffer[1] = '0';
	...
	if (extra_c)
		buffer[--ind] = extra_c;
	return (write(1, &buffer[ind], BUFF_SIZE - ind - ind - 1));
}
